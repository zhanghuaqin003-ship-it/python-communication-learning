# Python 通信工程项目学习轨迹

## 2026-09-10

- 学习主题：变量、赋值、加减运算、`print()`、`input()`、`float()`。
- 通信实践：链路预算接收功率计算。
- 已完成：独立计算 `-55 dBm`、`-65 dBm`、`-61 dBm`；完成发射功率与路径损耗的交互式输入。
- 当前状态：理解输入字符串需要转换为数字；正在巩固同时输入多个参数。
- 下一步：加入发射天线增益和接收天线增益；学习 `if` 条件判断，用接收功率门限判断链路是否可用。

## 2026-09-10（继续学习）

- 学习主题：`if/else` 条件判断与 `>=` 门限比较。
- 已完成：独立写出接收功率门限判断程序。
- 复习通过：能判断输入 `-80` 时 `-80 >= -80` 为 `True`，输出“链路可用”。
- 工具设置：关闭 VS Code 自动建议、触发字符补全、回车接受建议和参数提示。

## 2026-09-11

- 综合练习：加入发射天线增益和接收天线增益，计算接收功率 `-59 dBm`。
- 条件判断：正确判断链路可用。
- 注释：使用 `#` 添加说明；下一步继续区分注释与变量。

## 2026-09-11（字符串格式化）

- 学习主题：f-string。
- 已完成：用 `{rx_power_dbm}` 输出变量当前存储的数值，并区分接收功率变量 `rx_power_dbm` 与接收天线增益 `rx_gain_db`。
- 下一步：用 `for` 循环批量计算不同路径损耗下的接收功率。

## 2026-09-14（机器视觉学习计划）

- 已将四个月机器视觉入门计划同步到 `MACHINE_VISION_16_WEEK_PLAN.md`。
- 学习方式：每周拆成可勾选的小任务，代码和学习记录继续保存在本仓库。
- 今日主题：列表与 `for` 循环，开始把通信链路预算练习迁移为批量数据处理。
- 今日练习文件：`lessons/2026-09-14-lists-for.md`。
- 当前状态：待完成列表、循环、缩进和批量链路预算练习。

## 当前练习代码摘要

`@BT@@
print(f"接收功率是：{rx_power_dbm} dBm")
`@BT@@
